...
const ProdToOrder: React.VFC<{}> = () => {
  ...
  const total =
    prod?.count &&
    prod?.priceInDollar &&
    prod?.count * prod?.priceInDollar + shippingAlias;
    ...
    <section className='to-order-info'>
      {prod?.count === 1 ? (
        <section className='order-limit-case'>
          ...
          <p>
            You'r acceded the minimum order limit, You can order a simple
            instade:)
          </p>
          <AppButton
            value='Buy Simple?'
      ...
      <section>
        <p className='price-and-quantity'>
          ...
          <span className='value animate-up'>
            ${toFixedNumber(total && total - shippingAlias)}
          </span>
        </p>
